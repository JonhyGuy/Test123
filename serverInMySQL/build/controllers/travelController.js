"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.travelController = void 0;
const mysql_1 = __importDefault(require("../databases/mysql"));
class TravelController {
    ListTravel(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const travel = yield mysql_1.default.query('SELECT * FROM Travel');
            res.json(travel);
        });
    }
    createTravel(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            yield mysql_1.default.query('INSERT INTO Travel set ?', [req.body]);
            res.json({ message: 'Travel created' });
        });
    }
    deleteTravel(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            yield mysql_1.default.query('DELETE FROM Travel WHERE id = ?', [id]);
            res.json({ message: 'Travel where id = ' + id + ' delete' });
        });
    }
    updateTravel(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            yield mysql_1.default.query('UPDATE Travel set ? WHERE id = ?', [req.body, id]);
            res.json({ message: 'Travel where id = ' + id + ' update' });
        });
    }
}
exports.travelController = new TravelController();
